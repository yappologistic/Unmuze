...
1. Download the Linux package from the latest release, such as an `.AppImage` or `.deb`. An `.rpm` may also be available when generated by the release build.
...
Use **Check for updates** in Settings to install newer Tauri signature-verified Unmuze releases from inside the app.
